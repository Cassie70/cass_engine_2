...
Vector2<float> CameraController::ScreenToWorld(
  const Vector2<float> &screen, float width, float height
) {
...
void CameraController::HandleInputUpdate(
  ...
  if (Input::IsMousePressed(Mouse::Middle)) {
    ...
    velocity = Vector3<float>(direction, 0.0f).normalize() * 400;
...
void CameraController::HandleInputEvent(Event &e) {
  if (e.GetType() == EventType::WindowResize) {
    ...
    m_Camera.SetProjection(
      -resize.Width * 0.5f, resize.Width * 0.5f, -resize.Height * 0.5f,
      resize.Height * 0.5f
    );
